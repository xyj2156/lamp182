...
class PersonageController extends Controller
{
    ...
    public function getIndex(Request $request)
    {
    	...
    	session( ['member'=>Member::find(15)] );

    	$data = session('member')->detail;
    	
    	$data['username'] = session('member')['username'];
    	...
    	//非Ajax显示到页面 
    	if(!$request->ajax()) return view('home.personage.personage',['data'=>$data]);

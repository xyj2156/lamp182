            ...
            <div class="am-u-sm-12 am-u-md-12 am-u-lg-12">
                <div class="widget am-cf">
                    <div class="widget-head am-cf">
                        <div class="widget-title am-fl"><a href="{{url('admin/type')}}">分类管理 </a> > 添加分类</div>
                    </div>
                    <div class="widget-body am-fr">
                        <form class="am-form tpl-form-line-form" method="post" action="{{url('admin/type')}}">
                            {{csrf_field()}}
                                ...
                                <div class="am-u-sm-9">
                                    <select data-am-selected="{searchBox: 1}" style="display: none;" name="pid">
                                        @foreach(config('film.type') as $k => $v)
                                            <option value="{{$k}}" {{old('pid') == $k ? 'selected' : ''}}>{{$v}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="am-form-group">
                                ...
                                <div class="am-u-sm-9">
                                    <input type="text" name="name" value="{{old('name')}}" placeholder="请填写分类名">
                                </div>
                            </div>
                            <div class="am-form-group">
